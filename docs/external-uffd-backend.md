...
session with zero-copy response handling and can be configured as follows:
...
`UFFD_MODE_MISSING`. The Handshake leaves `MANAGED`, `PREFAULT`,
`ACK_REQUIRED`, and `BACKING_FDS` clear. Writable external PMEM and
backend-specific migration are not currently supported. `discard_writes` has
no effect when `readonly=on`.
...
5. Do not send a response for a successful Handshake.
...
regions. Cloud Hypervisor currently leaves `ACK_REQUIRED` clear, so a server
reports a Handshake error by closing the socket.
